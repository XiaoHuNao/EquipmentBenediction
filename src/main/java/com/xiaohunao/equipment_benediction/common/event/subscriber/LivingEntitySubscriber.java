...
public class LivingEntitySubscriber {
    ...
    public static void onLivingDamage(LivingDamageEvent.Pre event) {
        ...
        if(source.is(DamageTypeTags.IS_PLAYER_ATTACK)){
            ...
            if (weaponItem != null) {
                ModifierComponent modifierComponent = weaponItem.get(EBDataComponentTypes.MODIFIER);
                for (ModifierInstance modifierInstance : modifierComponent.modifierInstances()) {
                    Collection<BeforeMeleeHitHook> beforeMeleeHitHooks = modifierInstance.getModifier().getHookMap().get(EBHookTypes.BEFORE_MELEE_HIT.get());
