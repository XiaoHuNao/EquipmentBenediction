        ...
        for (Map.Entry<IBenediction, HookMap> entry : entityHookManager.getHooks().entrySet()) {
            ...
            for (T hook : hookMap.get(hookType)) {
                if(hook instanceof SpecialTimeHook specialTimeHook){
                    if (entityHookManager.containsSpecialTimeHook(owner, hook,executor)) {
                        continue;
                    }

                    entityHookManager.addSpecialTimeHook(new SpecialTimeHookWrapper(specialTimeHook, owner, executor));
                    continue;
                }
